...
def passGen():
    ...
    for x in range(0, user_number):
        myList.append(words[random.randint(0, len(words))])
    ...
    print("Reserving...")
    realPassword.append(password[::-1])

    print(f"Your password is {realPassword[0]}")

passGen()

#Question 2
valuePassword=realPassword[0]
def rep_with_upper(value):
    newValue = str(value).upper()
    realPassword.clear()
    return newValue

valueUpper=rep_with_upper(valuePassword)
print("--Rep With Upper--")
print(valueUpper)

def swap_letters(value):
    b=value[2:]
    c=value[:2]
    return b + c

valueSwap=swap_letters(valueUpper)
print("--Swap Letters--")
print(valueSwap)
...
def search_letter(source,key):
    if str(source).find(key)>0:
        return True
    else :
        return False
print("---------------")
if search_letter(valueSwap,"B"):
    print(f"B is in the {valueSwap}")
else:
    print(f"B is not in the {valueSwap}")
